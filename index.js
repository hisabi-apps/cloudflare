...
// 4. إرسال إشعار مخصص للمستخدم (قبول/رفض) — يسمح بالاختبار اليدوي
app.post('/api/notify', async (req, res) => {
  try {
    const { userId, approved, fileId, title, comment } = req.body;
    ...
    if (Array.isArray(deviceTokens) && deviceTokens.length > 0) {
      const messages = deviceTokens.map(token => ({
        ...
        data: { fileId: notificationData.fileId, approved: String(notificationData.approved) },
      }));

      const fcmResults = await Promise.allSettled(messages.map(msg => admin.messaging().send(msg)));
      const failures = fcmResults
        .map((r, i) => (r.status === 'rejected' ? { index: i, reason: r.reason?.message } : null))
        .filter(Boolean);

      return res.json({ success: true, notificationId: notifDocRef.id, fcmFailures: failures });
    }

    return res.json({ success: true, notificationId: notifDocRef.id, fcmFailures: [] });
  } catch (error) {
    console.error('Notify endpoint failed:', error);

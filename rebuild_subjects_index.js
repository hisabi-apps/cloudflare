...
(async () => {
  try {
    console.log('🔧 Rebuilding app_metadata.subjects_index from subject_stats / approved files...');
    const result = await subjectStatsService.rebuildSubjectsIndexFromSubjectStats({ batchSize: 500 });
    console.log('✅ Rebuild completed:', result);
    process.exit(0);
  } catch (error) {
    console.error('❌ Failed to rebuild subjects_index:', error.message || error);
    process.exit(1);
  }
})();
